...
function normalizeManager(m: string): string {
  // Merge iShares (BlackRock's ETF brand) into BlackRock per business rule.
...
function normalizeMaster(rows: Raw[]): MasterRow[] {
  return rows
...
function normalizeDisplacement(rows: Raw[]): DisplacementRow[] {
...
export const useDataLoader = create<DataState>((set, get) => ({
  ...
  load: async () => {
    ...
    try {
      ...
      set((st) => ({ status: "ready", version: st.version + 1 }));
    } catch (e) {
